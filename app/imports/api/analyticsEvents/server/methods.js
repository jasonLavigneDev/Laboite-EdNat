...
export const analyticsConnectionsCounts = new ValidatedMethod({
  ...
  run({ structureId }) {
    ...
    const result = {
      ...
      notLogged: UserStatus.connections.find({ userId: { $exists: false } }).count(),
      idle: Meteor.users.find({ ...additionalRequest, 'status.idle': true }).count(),
      active: Meteor.users.find({ ...additionalRequest, 'status.idle': false }).count(),
...
export const getActionClickedAnalyticsEvents = new ValidatedMethod({
  ...
  run({ structureId }) {
    ...
    const data = AnalyticsEvents.aggregate([
      {
        $match: structure ? { structureId: { $in: structureArrayWithChilds } } : {},
      },
      {
        $group: {
          _id: '$content',
