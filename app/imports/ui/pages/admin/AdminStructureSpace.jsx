...
import StructureSpaces from '../../../api/structurespaces/structurespaces';
...
import { updateStructureSpace } from '../../../api/structurespaces/methods';
...
const AdminStructureSpace = () => {
  ...
  const { services, loading, structures, structurespace, servicesList } = useTracker(() => {
    ...
    const structureHandle = Meteor.subscribe('structurespaces.one', {
      structureId: selectedStructureId || userStructure._id,
    });
    const allGlobalServicesHandle = Meteor.subscribe('services.all');
    const space = StructureSpaces.findOne({ structureId: selectedStructureId || userStructure._id });
    ...
    if (space) {
      space.sorted.forEach(({ elements = [] }) => {
        if (elements) {
          servicesAdded = [
            ...servicesAdded,
            ...services.concat(elements.filter((item) => item.type === 'service').map((service) => service.element_id)),
          ];
        }
      });
    }
